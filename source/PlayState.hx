...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		// startCountdown();
		if (SONG.hasExtraParts) // preloads without calcing length
		{
			for (i in 0...SONG.maxParts)
			{
				Paths.inst(SONG.song.toLowerCase(), i);
				Paths.voices(SONG.song.toLowerCase(), i);
			}
		}
	...
	public function setSongTime(time:Float)
	{
		...
		FlxG.sound.music.time = time - extraPart;
		FlxG.sound.music.play();

		if (Conductor.songPosition - extraPart <= vocals.length)
		{
			vocals.time = time - extraPart;
		}
		vocals.play();
	...
	function startSong():Void
	{
		...
		var curSongLength = 0.0;
		if (SONG.hasExtraParts) // preloading? (not really this calcs the length)
		{
			for (i in 0...maxParts + 1)
			{
				FlxG.sound.playMusic(Paths.inst(SONG.song.toLowerCase(), i));
				curSongLength += FlxG.sound.music.length;
				FlxG.sound.music.stop();
				FlxG.sound.music.destroy();
			}
		}
		FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song, curPart), 1, false);
	...
	function resyncVocals():Void
	{
		...
		if (Conductor.songPosition - extraPart <= vocals.length)
		{
			vocals.time = Conductor.songPosition - extraPart;
		}
		vocals.play();
	}
	...
	public function endSong():Void
	{
		if (moreThanOnePart && curPart < maxParts)
		{
			...
			FlxG.sound.playMusic(Paths.inst(SONG.song.toLowerCase(), curPart), false);
			FlxG.sound.music.onComplete = onSongComplete;
			vocals = new FlxSound().loadEmbedded(Paths.voices(SONG.song.toLowerCase(), curPart));
	...
	private function keyShit():Void
	{
		...
		if (startedCountdown && !boyfriend.stunned && generatedMusic)
		{
			...
			if (boyfriend.holdTimer > Conductor.stepCrochet * 0.0011 * boyfriend.singDuration
				&& boyfriend.animation.curAnim.name.startsWith('sing')
				&& !boyfriend.animation.curAnim.name.endsWith('miss'))
			{
				boyfriend.dance();
