...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var check_extraParts = new FlxUICheckBox(175, 180, null, null, "Has extra parts", 100);
		check_extraParts.checked = _song.hasExtraParts;
		check_extraParts.callback = function()
		{
			_song.hasExtraParts = check_extraParts.checked;
			moreThanOnePart = _song.hasExtraParts;
			if (moreThanOnePart)
				curPart = 0;
			else
				curPart = -1;
		};
		check_extraParts.callback();

		var stepperMaxParts:FlxUINumericStepper = new FlxUINumericStepper(175, 230, 1, 1, 1, 400, 3);
		stepperMaxParts.value = _song.maxParts;
		stepperMaxParts.name = 'song_maxParts';
		blockPressWhileTypingOnStepper.push(stepperMaxParts);

		tab_group_song.add(new FlxText(stepperMaxParts.x, stepperMaxParts.y - 15, 0, 'Max Parts:'));
		tab_group_song.add(stepperMaxParts);
		tab_group_song.add(check_extraParts);
	...
	var extraPart = 0.0;
	var calcLength = 0.0; // added lengths
	...
	function reloadAudio(takeAway = false)
	{
		...
		vocals.time = FlxG.sound.music.time; // should be 0 but eh
		vocals.pause();
	...
	{
		if (_song.hasExtraParts) // preload
		{
			calcLength = 0.0;
			for (i in 0..._song.maxParts + 1)
			{
				FlxG.sound.playMusic(Paths.inst(_song.song.toLowerCase(), i));
				calcLength += FlxG.sound.music.length;
				FlxG.sound.music.stop();
				FlxG.sound.music.destroy();
				Paths.voices(_song.song.toLowerCase(), i);
			}
		}
		reloadAudio();
	}
			...
			if (FlxG.mouse.wheel != 0)
			{
				...
				else
				{
					...
					if (FlxG.mouse.wheel > 0)
					{
						...
						FlxG.sound.music.time = Conductor.beatToSeconds(fuck) - extraPart;
					}
					else
					{
						var fuck:Float = CoolUtil.quantize(beat, snap) + increase;
