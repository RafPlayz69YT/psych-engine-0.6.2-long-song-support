...
class PauseSubState extends MusicBeatSubstate
{
		...
		switch (daSelected)
		{
			case 'Skip Time':
				...
				if (controls.UI_LEFT || controls.UI_RIGHT)
				{
					...
					if (holdTime > 0.5)
					{
						curTime += 45000 * elapsed * (controls.UI_LEFT ? -1 : 1) * (FlxG.keys.pressed.SHIFT ? 4 : 1);
					}
	...
	function updateSkipTimeText()
	{
		...
		skipTimeText.text = FlxStringUtil.formatTime(Math.max(0, Math.floor(curTime / 1000)), false)
			+ ' / '
			+ FlxStringUtil.formatTime(Math.max(0, Math.floor(PlayState.instance.songLength / 1000)), false);
	}
}
